...
def progress(operation, value):
    ...
    if value == 0:
        ...
        status = f' progress ({value} % completed)'
...
def main_menu():
    ...
    while True:
        ...
        try:
            choice = int(input("choose option 1-4 from the menu"))
            if 1 <= choice <=4:
                return choice
            else:
                error("choose from 1 to 4")
        except:
            error("enter an intger")
...
.00
    ...
    while True:
        try:
            response= int(input("give response"))    
            if variant == 1 and 1 <= response <= 4:
                return response
            elif variant == 2 and 1 <= response <= 3:
                return response
            elif variant == 3 and 1 <= response <= 2:
                return response
            else:
...
def total_reviews(num_reviews):
    ...
    """
    # TODO: Your code here
    print( f'there are {num_reviews} in the data set')
    pass # can remove
...
def review_dates():
    ...
    dates = []
    while True:
        '''review_dates = input("Enter review dates (mm/dd/yyyy), separated by commas: ").split(",")
    review_dates = [date.strip() for date in review_dates]
    return review_dates'''
        dates = input("Enter a review date separated in commas in the format of (mm/dd/yyyy)").split()
...
def display_review(review, cols=None):
    ...
    if cols is None or len(cols) == 0:
        cols = range(len(review))
    else:
        selected_values = [review[i] for i in cols]
    print(selected_values)

# Test the display_review function
#sample_review = ["08/03/2017", "Hotel Arena", "Russia", "I am so angry...", "Only the park...", 2.9, 7, "['Leisure trip', 'Couple', 'Duplex Double Room', 'Stayed 6 nights']", "0 days"]

# Display all values
#display_review(sample_review)

# Display values at specific column indexes
#display_review(sample_review, [1, 3])

# Display values at specific column indexes
#display_review(sample_review, [0, 1, 5])
...
def display_reviews(reviews, cols=None ):
    ...
    for review in reviews:
        if cols is None or not cols:
            ...
            display_review = [review[col] for col in cols]
        print(display_review)
            
# Test the display_reviews function
'''sample_reviews = [
    ["08/03/2017", "Hotel Arena", "Russia", "I am so angry...", "Only the park...", 2.9, 7, "['Leisure trip', 'Couple', 'Duplex Double Room', 'Stayed 6 nights']", "0 days"],
    ["07/15/2017", "Hotel Arena", "Russia", "Not bad...", "Nothing special...", 7.5, 4, "['Leisure trip', 'Couple', 'Duplex Double Room', 'Stayed 3 nights']", "1 day"]
]

# Display all values for each review
display_reviews(sample_reviews)

# Display values at specific column indexes for each review
display_reviews(sample_reviews, [1, 3])

# Display values at specific column indexes for each review
display_reviews(sample_reviews, [0, 1, 5])
  #  pass # can remove'''
...
class ReviewExporter:
    ...
    def export_to_json(self, filename):
        # Code for exporting reviews to JSON file
        # Use self.reviews to access the reviews to be exported
        # Use the filename parameter to specify the output file name

        exporter = ReviewExporter(display_review)

    ''' if export_option == 1:
         exporter.export_to_json("all_reviews.json")  # Export all reviews
        elif export_option == 2:
            exporter.export_to_json("filtered_reviews.json")  # Export filtered reviews
        else:
            tui.error("Invalid export option. Please try again.")
